...
function(file, flags = FALSE, add.elem, format, verbose = TRUE) {
	...
	if(verbose) {
		message(paste(
			gettextf("Format is %s with %i records from %i station%s,\nread in %.1f seconds\n",
				format, nrow(d), length(levels(d$id)),
				ifelse(length(levels(d$id))>1,"s",""),
				read.time[3])),
			gettextf("%i element%s found: %s\n",
				length(elem.name), ifelse(length(elem.name)>1,"s",""),
				paste(elem.name,collapse=", ")),collapse="\n")
	}
	if(flags) {
