...
function(x, start, end, id, precip.only = FALSE, maxy, ylim,
	...
	barplot(pl,space=0,names.arg=levels(dat$year),col=col,ylab=ylab1,ylim=c(0,maxy))
	title(main=main$title,line=main$line)
	axis(1,1:years-.5,FALSE)
	na.h <- maxy/60
	rect(1:years-.5-precip.na/2,0,1:years-.5+precip.na/2,na.h,col="red",border=FALSE)
	...
	boxplot(mean_t ~ year, dat, varwidth=TRUE,col="grey",ylim=ylim,ylab=ylab2,xaxt="n",bty="n")
	axis(1,1:years,FALSE)
	axis(3,1:years,FALSE)
	abline(h=0)
	na.h <- diff(ylim)/80
	rect(1:years-temp.na/2,-na.h,1:years+temp.na/2,na.h,col="red",border=FALSE)
}
